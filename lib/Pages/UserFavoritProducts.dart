...
class _UserFavoriteProductsState extends State<UserFavoriteProducts> {
  ...
  void load() {
    ...
    NetworkManager.httpGet(
        Globals.baseUrl + "product/my/favourite/${UserManager.currentUser("id")}", context, (r) { // product/load?page=$page&type=favorit
      setState(() {
        isLoading = false;
      });
      if (r['state'] == true) {
  ...
  Widget getProducts() {
    ...
    if(items.length == 1)
      items.add((Container(width: MediaQuery.of(context).size.width * 0.5)));
  ...
  // Widget createProductItem(item, i) {
  //   double size = 120;
  //   return Container(
